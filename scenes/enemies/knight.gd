...
func check_collisions() -> void:
	...
	if raycast_left.is_colliding():
		if raycast_left.get_collider().is_in_group('player') or raycast_left.get_collider().is_in_group('goblin'):
			attack(raycast_left.get_collider(), 'Left')
	elif raycast_down.is_colliding():
		if raycast_down.get_collider().is_in_group('player') or raycast_down.get_collider().is_in_group('goblin'):
			attack(raycast_down.get_collider(), 'Down')
	elif raycast_up.is_colliding():
		if raycast_up.get_collider().is_in_group('player') or raycast_up.get_collider().is_in_group('goblin'):
			attack(raycast_up.get_collider(), 'Up')
	elif raycast_right.is_colliding():
		if raycast_right.get_collider().is_in_group('player') or raycast_right.get_collider().is_in_group('goblin'):
